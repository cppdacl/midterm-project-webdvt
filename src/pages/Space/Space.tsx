...
export default function Space() {
  ...
  const { user, spaces, bookings } = useApp()!;
  const navigate = useNavigate();

  const space = spaces.find((s) => s.id === Number(spaceId)) as
    | SpaceType
    | undefined;
  ...
  useEffect(() => {
    if (user && space) {
      const booked = bookings.some(
        (b) => b.spaceId === space.id && b.username === user,
      );
  ...
  return (
    <div className="space-page">
        ...
        <div className="space-info-split">
          <div className="time-slots">
            {space.time_slots.map((slot, index) => (
              <span className="time-slot-tag" key={index}>
                {slot}
              </span>
            ))}
          </div>
          <div className="price-label">₱{space.price} / Slot</div>
        </div>

        {alreadyBooked ? (
          <button className="book-btn cancel" onClick={handleCancelBooking}>
            See Booking
          </button>
        ) : (
          <button
      ...
      {showModal && !alreadyBooked && (
        <BookForm space={space} onClose={() => setShowModal(false)} />
      )}
    </div>
  );
}
